...
public final class SolidProvider {
     ...
     */
    public static @NotNull Solid fromPoints(@NotNull Vec3D min, @NotNull Vec3D max) {
    ...
    private static Solid[] getSplitFor(Shape tallShape) {
        ...
        Solid[] newSplit = new Solid[] {fromPoints(VecUtils.toDouble(start), Vec3D.immutable(end.x(), 1, end.z())),
                fromPoints(Vec3D.immutable(start.x(), 0, start.z()), Vec3D.immutable(end.x(), end.y() - 1, end.z()))};
