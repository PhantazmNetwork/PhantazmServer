...
public class SpatialCollider implements Collider {
    ...
    private static boolean checkSolid(SolidPipe overlapping, Solid component, double centerX, double centerY,
        ...
        Vec3D componentMin = component.getMin();
        Vec3D componentMax = component.getMax();

        double minX = overlapping.getFirst() + componentMin.x() - centerX;
        double minY = overlapping.getThird() + componentMin.y() - centerY;
        double minZ = overlapping.getSecond() + componentMin.z() - centerZ;

        double maxX = overlapping.getFirst() + componentMax.x() - centerX;
        double maxY = overlapping.getThird() + componentMax.y() - centerY;
        double maxZ = overlapping.getSecond() + componentMax.z() - centerZ;
    ...
    private static boolean checkAxis(double size, double dA, double dB, double minA, double minB, double maxA,
        ...
        return dA * dB <= 0
               ? checkPlanes(size, dA, dB, minA, minB, maxA, maxB)
    ...
    private double collisionCheck(double x, double y, double z, double width, double height, double depth, double dX,
        ...
        if (overlapping.hasNext()) {
            ...
            do {
                ...
                Vec3D candidateMin = candidate.getMin();

                double moX = overlapping.getFirst() + candidateMin.x();
                double moY = overlapping.getThird() + candidateMin.y();
                double moZ = overlapping.getSecond() + candidateMin.z();
    ...
    public double highestCollisionAlong(double x, double y, double z, double width, double height, double depth,
        ...
        return collisionCheck(x, y, z, width, height, depth, dX, dY, dZ, Double.NEGATIVE_INFINITY,
                (solid, val) -> val + solid.getMax().y(), (value, best) -> value > best,
                solid -> solid.getMax().y() == 1.0F, val -> val < 0);
    }
    ...
    public double lowestCollisionAlong(double x, double y, double z, double width, double height, double depth,
        ...
        return collisionCheck(x, y, z, width, height, depth, dX, dY, dZ, Double.POSITIVE_INFINITY,
                (solid, val) -> val + solid.getMin().y(), (value, best) -> value < best,
                solid -> solid.getMin().y() == 0F, val -> val > 0);
    }

    @Override
    public double heightAt(int x, int y, int z) {
        Solid solid = space.solidAt(x, y, z);
        return solid == null ? y : y + solid.getMax().y();
    }
