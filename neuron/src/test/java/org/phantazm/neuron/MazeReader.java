                ...
                for (int x = 0; x < width; x++) {
                    for (int y = 0; y < height; y++) {
                        ...
                        if (color.equals(Color.RED) && start == null) {
                            ...
                            solids.add(Vec3I.immutable(x, 0, y)); //wall nodes are black
                        }
                    }
                }
                ...
                if (correctPath != null) {
                    try (correctPath) {
                        ...
                        Vec3I[] correctPathElements = root.getElement("points").asList().stream().map(element -> {
                            ...
                            return Vec3I.immutable(vectors.get(0).asNumber().intValue(), 0,
                                    vectors.get(1).asNumber().intValue());
                        }).toArray(Vec3I[]::new);

                        return new Data(solids, Vec3I.immutable(start.getX, 0, start.getY),
                                Vec3I.immutable(end.getX, 0, end.getY), correctPathElements);
                    }
                }
    ...
    private static int[][] loadGridFromRaster(Raster raster) {
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                grid[i][j] =
                        raster.getSample(i, j, 0) << 16 | (raster.getSample(i, j, 1) << 8) | raster.getSample(i, j, 2);
            }
        }
